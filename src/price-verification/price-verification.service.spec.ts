...
describe('PriceVerificationService', () => {
  ...
  describe('recalculateTrustScore', () => {
    const USER_ID = 'user-score-uuid';

    function makeVerification(result: VerificationResult): PriceVerification {
      return {
        id: `v-${Math.random()}`,
        price: mockPrice,
        verifier: mockVerifier,
    ...
    it('CONFIRMED 3건 / 전체 5건 → 점수 60', async () => {
      const verifications = [
        makeVerification(VerificationResult.CONFIRMED),
        makeVerification(VerificationResult.CONFIRMED),
        makeVerification(VerificationResult.CONFIRMED),
        makeVerification(VerificationResult.DISPUTED),
        makeVerification(VerificationResult.DISPUTED),
      ];
      verificationRepository.find.mockResolvedValue(verifications);
    ...
    it('최근 90일 기준 MoreThan 조건으로 verificationRepository.find 호출', async () => {
      ...
      expect(verificationRepository.find).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            verifier: { id: USER_ID },
      ...
      // 전달된 createdAt 값이 90일 전 범위인지 검증
      ...
      expect(since90dValue.getTime()).toBeLessThanOrEqual(
        expected90dAgoAfter.getTime() + 5000,
      );
    });
    ...
    it('반환값은 void (undefined)', async () => {
      ...
      const result = await service.recalculateTrustScore(USER_ID);

      expect(result).toBeUndefined();
    });

    it('CONFIRMED 1건 / 전체 1건 → 점수 100', async () => {
      verificationRepository.find.mockResolvedValue([
        makeVerification(VerificationResult.CONFIRMED),
      ]);

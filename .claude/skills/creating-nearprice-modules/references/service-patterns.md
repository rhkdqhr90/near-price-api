...
> **`TrustScoreScheduler`(매일 03:00 배치)만 `User.trustScore`를 업데이트하는 단일 writer다.**
>
> 어떤 Service(PriceVerificationService, PriceReactionService 등)에서도
> `trustScore` 즉시 계산 메서드(`recalculateTrustScore`, `calculateAndUpdate` 등)를
> 추가하거나 호출하지 말 것. 공식 충돌 및 덮어쓰기 발생.
>
> 실제 구현은 `src/trust-score/` 모듈과 `PROJECT.md` 참조.

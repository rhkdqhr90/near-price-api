...
아래 5단계를 **전부 통과하기 전까지** 사용자에게 완료 보고를 할 수 없다.
...
## Step 3. test-writer Agent QA (Service·Controller 변경 시 필수)

Step 2 통과 후 **반드시** `test-writer` Agent를 호출한다.

**호출 조건**: 이번 작업에 `.service.ts` 또는 `.controller.ts` 파일이 포함된 경우. DTO/Entity/Module만 변경된 경우 생략 가능.

```
대상: 이번 작업에서 신규 생성하거나 수정한 .service.ts / .controller.ts 파일
방법: Agent tool, subagent_type=test-writer
```

### test-writer 호출 시 프롬프트 형식

```
다음 파일들의 테스트를 작성해줘:
- [변경된 service/controller 파일 목록]

요구사항:
1. Service 단위 테스트 (.service.spec.ts) — 모든 public 메서드
2. 성공/실패/엣지 케이스 포함 (NotFoundException, ForbiddenException, ConflictException)
3. TypeORM Repository는 jest.fn() mock 사용
4. 작성 후 npm test -- <파일명> 실행하여 통과 확인
5. 커버리지 목표: Service 80% 이상
```

### test-writer 결과 처리 규칙

| 결과 | 처리 방법 |
|------|-----------|
| 테스트 실패 | 즉시 수정 → 재실행 (최대 3회) |
| 커버리지 70% 미만 | 테스트 추가 → 재실행 |
| 테스트 통과 | 다음 단계 진행 |
...
nestjs-reviewer + test-writer 통과 후 변경 파일 기준으로 **반드시** 이 항목들을 직접 확인:
...
- test-writer: ✅ 통과 (커버리지 n%) 또는 ⏭️ 생략 (DTO/Entity만 변경)
